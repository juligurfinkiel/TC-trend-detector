...
const CATEGORIES = [
  { id: "memes", label: "Memes / Chistes", emoji: "\uD83D\uDE02", color: "#FF6B6B" },
  { id: "audios", label: "Audios Virales", emoji: "\uD83C\uDFB5", color: "#845EF7" },
  { id: "challenges", label: "Challenges", emoji: "\uD83D\uDD25", color: "#FF922B" },
];

const COUNTRIES = [
  { id: "ar", label: "Argentina", emoji: "\uD83C\uDDE6\uD83C\uDDF7" },
  { id: "cl", label: "Chile", emoji: "\uD83C\uDDE8\uD83C\uDDF1" },
  { id: "co", label: "Colombia", emoji: "\uD83C\uDDE8\uD83C\uDDF4" },
  { id: "br", label: "Brasil", emoji: "\uD83C\uDDE7\uD83C\uDDF7" },
];
...
function extractJSON(text) {
  ...
  var bracketStart = clean.indexOf("[");
  ...
  if (braceStart !== -1) {
    ...
    if (end2 !== -1) return JSON.parse(clean.slice(braceStart, end2 + 1));
  }
  throw new Error("No se encontro JSON valido en la respuesta");
}

function getTextFromResponse(data) {
  return (data.content || []).filter(function(b) { return b.type === "text" && b.text; }).map(function(b) { return b.text; }).join("\n");
}
